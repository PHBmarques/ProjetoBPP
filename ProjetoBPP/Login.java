...
public class Login {
    ...
    // 1. Testa o Login
    boolean loginValido = validarLogin(usuarioParaTestar, senhaParaTestar);
    
    if (loginValido) {
        System.out.println("RESULTADO: Login autorizado!");
    } else {
        System.out.println("RESULTADO: Acesso negado. Verifique usuário e senha.");
    }

    // 2. Testa a força da senha (independente do login)
    System.out.println("\nAnálise de Segurança da Senha");
    if(validarSenhaForte(senhaParaTestar)){
        System.out.println("A senha esta forte");
    }else{
        System.out.println("A senha esta fraca");
    }
